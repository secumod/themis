...
/*
 * Secure Comparator involves a whole raft of ideas that are not trivial for
 * developers without
 * deep knowledge of cryptography, in particular:
 *
 *  - Socialist Millionaire's Problem (SMP) -
 ...
 *
 * There are some dangerous situations when one of the communicating peers is
 * not fair and presents
 * forged secret or some of intermediate parameters. These may cause security
 * flaws. See original paper
 * with SMP solution and our GitHub issues
 * https://github.com/cossacklabs/themis/issues/85 and
 * https://github.com/cossacklabs/themis/issues/83.
 *
 * To prevent these situations, additional security proofs and verifications
 * should be done on each step of the protocol.
 * These proofs are implemented in functions below:
 *
 * ed_sign / ed_verify                   -> Proof of knowledge of EC discrete
 * logarithm (section 5.3.1 in paper);
 * ed_dbl_base_sign / ed_dbl_base_verify -> Proof of knowledge of EC discrete
 * coordinates (section 5.3.2 in paper);
 * ed_point_sign / ed_point_verify       -> Proof of knowledge of EC discrete
 * logarithms (section 5.3.3 in paper).
 *
 * All sign/verify operations are numerated (first parameter in each function)
 * to strictly follow original ED25519 algorithm.
 *
 *
 * Variables and values in functions that implement proofs correspond to initial
 * implementation of ED25519 signature
 * of D.J. Bernstein team.
 *
 * All variables in comments correspond to variables in Secure Comparator paper
 * but may slightly differ from variables in code.
 *
 */
...
secure_comparator_alice_step1(secure_comparator_t *comp_ctx, void *output,
                              size_t *output_length);
static themis_status_t
secure_comparator_bob_step2(secure_comparator_t *comp_ctx, const void *input,
                            size_t input_length, void *output,
                            size_t *output_length);
static themis_status_t
secure_comparator_alice_step3(secure_comparator_t *comp_ctx, const void *input,
                              size_t input_length, void *output,
                              size_t *output_length);
static themis_status_t
secure_comparator_bob_step4(secure_comparator_t *comp_ctx, const void *input,
                            size_t input_length, void *output,
                            size_t *output_length);
static themis_status_t
secure_comparator_alice_step5(secure_comparator_t *comp_ctx, const void *input,
                              size_t input_length, void *output,
                              size_t *output_length);

#define HASH_LENGTH 64 // for sha512
#define TEMP_BUFFER_LENGTH                                                     \
  ((ED25519_GE_LENGTH > HASH_LENGTH) ? ED25519_GE_LENGTH : HASH_LENGTH)
...
static themis_status_t ed_sign(uint8_t pos, const uint8_t *scalar,
                               uint8_t *signature) {
...
static themis_status_t ed_verify(uint8_t pos, const ge_p3 *point,
  ...
  ge_double_scalarmult_vartime(&R, signature, &A,
...
static themis_status_t ed_dbl_base_sign(uint8_t pos, const uint8_t *scalar1,
                                        const uint8_t *scalar2,
                                        const ge_p3 *base1, const ge_p3 *base2,
                                        uint8_t *signature) {
...
static themis_status_t ed_dbl_base_verify(uint8_t pos, const ge_p3 *base1,
                                          const ge_p3 *base2,
                                          const ge_p3 *point1,
                                          const ge_p3 *point2,
                                          const uint8_t *signature) {
  ...
  ge_double_scalarmult_vartime((ge_p2 *)&R1,
                               signature + (2 * ED25519_GE_LENGTH), base1,
                               signature + ED25519_GE_LENGTH);
...
static themis_status_t ed_point_sign(uint8_t pos, const uint8_t *scalar,
                                     const ge_p3 *point, uint8_t *signature) {
  uint8_t r[ED25519_GE_LENGTH];
  ge_p3 R;
  uint8_t k[TEMP_BUFFER_LENGTH];
...
static themis_status_t ed_point_verify(uint8_t pos, const ge_p3 *base2,
  ...
  ge_double_scalarmult_vartime((ge_p2 *)&R1, signature, &p_neg,
                               signature + ED25519_GE_LENGTH);
...
themis_status_t secure_comparator_append_secret(secure_comparator_t *comp_ctx,
  ...
  soter_status =
      soter_hash_update(&(comp_ctx->hash_ctx), secret_data, secret_data_length);
  return (themis_status_t)soter_status;
}
...
secure_comparator_alice_step1(secure_comparator_t *comp_ctx, void *output,
  ...
  *output_length = 6 * ED25519_GE_LENGTH;
  ...
  themis_status = (themis_status_t)soter_hash_final(
      &(comp_ctx->hash_ctx), comp_ctx->secret, &secret_length);
  ...
  themis_status = ed_sign(1, comp_ctx->rand2,
                          ((unsigned char *)output) + ED25519_GE_LENGTH);
  if (THEMIS_SUCCESS != themis_status) {
    return themis_status;
  }
  ...
  themis_status = ed_sign(2, comp_ctx->rand3,
                          ((unsigned char *)output) + (4 * ED25519_GE_LENGTH));
  if (THEMIS_SUCCESS != themis_status) {
    return themis_status;
  }
...
secure_comparator_bob_step2(secure_comparator_t *comp_ctx, const void *input,
  ...
  /* Input validation from Alice's 1 step (amount of group elements in
   * brackets):
  ...
  if (ge_frombytes_vartime(&(comp_ctx->g3p), ((const unsigned char *)input) +
                                                 (3 * ED25519_GE_LENGTH))) {
    return THEMIS_INVALID_PARAMETER;
  }

  /* Output will contain 4 group elements and 2 * 2 + 3 ZK-proofs, so 4 + 2 * 2
   * + 3 = 11 */
  ...
  themis_status =
      ed_verify(1, &g2a, (const unsigned char *)input + ED25519_GE_LENGTH);
  ...
  themis_status = ed_verify(2, &(comp_ctx->g3p), (const unsigned char *)input +
                                                     (4 * ED25519_GE_LENGTH));
  if (THEMIS_INVALID_SIGNATURE == themis_status) {
    comp_ctx->result = THEMIS_SCOMPARE_NO_MATCH;
  } else if (THEMIS_SUCCESS != themis_status) {
  ...
  *output_length = 11 * ED25519_GE_LENGTH;
  ...
  ge_double_scalarmult_vartime((ge_p2 *)&(comp_ctx->Q), comp_ctx->secret,
                               &(comp_ctx->g2), comp_ctx->rand);
  ...
  themis_status = ed_sign(3, comp_ctx->rand2,
                          ((unsigned char *)output) + ED25519_GE_LENGTH);
  if (THEMIS_SUCCESS != themis_status) {
    return themis_status;
  }
  ...
  themis_status = ed_sign(4, comp_ctx->rand3,
                          ((unsigned char *)output) + (4 * ED25519_GE_LENGTH));
  if (THEMIS_SUCCESS != themis_status) {
    return themis_status;
  }
  ...
  ge_p3_tobytes(((unsigned char *)output) + (6 * ED25519_GE_LENGTH),
                &(comp_ctx->P));

  /* Copy Qb (1 group element) as byte array to output */
  ge_p3_tobytes(((unsigned char *)output) + (7 * ED25519_GE_LENGTH),
                &(comp_ctx->Q));

  /* Signature of Qb (3 group elements) is copied to output */
  themis_status = ed_dbl_base_sign(
      5, comp_ctx->rand, comp_ctx->secret, &(comp_ctx->g2), &(comp_ctx->g3),
   ...
   * G2b || G2b signature || G3b || G3b signature || Pb || Qb || Pb + Qb
   * signature
...
secure_comparator_alice_step3(secure_comparator_t *comp_ctx, const void *input,
   ...
   * G2b (1) || G2b signature (2) || G3b (1) || G3b signature (2) || Pb (1) ||
   * Qb (1) || Pb + Qb signature (3)
   * */
  ...
  if (ge_frombytes_vartime(&(comp_ctx->Pp), ((const unsigned char *)input) +
                                                (6 * ED25519_GE_LENGTH))) {
    return THEMIS_INVALID_PARAMETER;
  }
  if (ge_frombytes_vartime(&Qb, ((const unsigned char *)input) +
                                    (7 * ED25519_GE_LENGTH))) {
    return THEMIS_INVALID_PARAMETER;
  }
  ...
  /* Verification of G2b signature */
  themis_status =
      ed_verify(3, &g2b, (const unsigned char *)input + ED25519_GE_LENGTH);
  if (THEMIS_INVALID_SIGNATURE == themis_status) {
    comp_ctx->result = THEMIS_SCOMPARE_NO_MATCH;
  } else if (THEMIS_SUCCESS != themis_status) {
    return themis_status;
  ...
  themis_status = ed_verify(4, &(comp_ctx->g3p), (const unsigned char *)input +
                                                     (4 * ED25519_GE_LENGTH));
  if (THEMIS_INVALID_SIGNATURE == themis_status) {
    comp_ctx->result = THEMIS_SCOMPARE_NO_MATCH;
  } else if (THEMIS_SUCCESS != themis_status) {
  ...
  *output_length = 8 * ED25519_GE_LENGTH;
  ...
  themis_status = ed_dbl_base_verify(
      5, &(comp_ctx->g2), &(comp_ctx->g3), &(comp_ctx->Pp), &Qb,
  ...
  themis_status = ed_dbl_base_sign(
      6, comp_ctx->rand, comp_ctx->secret, &(comp_ctx->g2), &(comp_ctx->g3),
  ...
  themis_status =
      ed_point_sign(7, comp_ctx->rand3, &(comp_ctx->Qa_Qb),
...
secure_comparator_bob_step4(secure_comparator_t *comp_ctx, const void *input,
  ...
  /* Input validation from Alice's 3 step (amount of group elements in
   * brackets):
  ...
  if (ge_frombytes_vartime(&Qa, ((const unsigned char *)input) +
                                    ED25519_GE_LENGTH)) {
    return THEMIS_INVALID_PARAMETER;
  }
  if (ge_frombytes_vartime(&Ra, ((const unsigned char *)input) +
                                    (5 * ED25519_GE_LENGTH))) {
    return THEMIS_INVALID_PARAMETER;
  }
  ...
  *output_length = 3 * ED25519_GE_LENGTH;
  ...
  themis_status =
      ed_dbl_base_verify(6, &(comp_ctx->g2), &(comp_ctx->g3), &Pa, &Qa,
  ...
  themis_status =
      ed_point_verify(7, &(comp_ctx->g3p), &Qa, &Ra,
  ...
  if (THEMIS_SCOMPARE_NOT_READY == comp_ctx->result) {
    comp_ctx->result =
        ge_cmp(&Rab, &Pa_Pb) ? THEMIS_SCOMPARE_NO_MATCH : THEMIS_SCOMPARE_MATCH;
  }
  ...
  themis_status = ed_point_sign(8, comp_ctx->rand3, &Qa,
                                ((unsigned char *)output) + ED25519_GE_LENGTH);
  if (THEMIS_SUCCESS != themis_status) {
    return themis_status;
  }
   ...
   * Rb || Rb signature
   * Bob proceeds 4 step, finishs and gets result, Alice proceeds 5 step,
   * finishes and gets result.
   */
...
themis_status_t secure_comparator_begin_compare(secure_comparator_t *comp_ctx,
  ...
  return secure_comparator_alice_step1(comp_ctx, compare_data,
                                       compare_data_length);
}

themis_status_t secure_comparator_proceed_compare(
    secure_comparator_t *comp_ctx, const void *peer_compare_data,
    size_t peer_compare_data_length, void *compare_data,
    size_t *compare_data_length) {
  ...
  if (comp_ctx->state_handler) {
    ...
    return comp_ctx->state_handler(comp_ctx, peer_compare_data,
                                   peer_compare_data_length, compare_data,
                                   compare_data_length);
  }

  /* This is initial proceed call. "Accept" the protocol. */
  return secure_comparator_bob_step2(comp_ctx, peer_compare_data,
                                     peer_compare_data_length, compare_data,
                                     compare_data_length);
}

themis_status_t
secure_comparator_get_result(const secure_comparator_t *comp_ctx) {
  if (!comp_ctx) {
    return THEMIS_INVALID_PARAMETER;
  }
