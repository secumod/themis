...
themis_status_t themis_gen_key_pair(soter_sign_alg_t alg, uint8_t *private_key,
  ...
  soter_status_t res =
      soter_sign_export_key(ctx, private_key, private_key_length, true);
  if (res != THEMIS_SUCCESS && res != THEMIS_BUFFER_TOO_SMALL) {
    soter_sign_destroy(ctx);
    return res;
  }
  soter_status_t res2 =
      soter_sign_export_key(ctx, public_key, public_key_length, false);
...
themis_status_t themis_gen_rsa_key_pair(uint8_t *private_key,
                                        ...
                                        uint8_t *public_key,
                                        size_t *public_key_length) {
  soter_rsa_key_pair_gen_t *key_pair_ctx =
      soter_rsa_key_pair_gen_create(THEMIS_RSA_KEY_LENGTH);
  themis_status_t res = soter_rsa_key_pair_gen_export_key(
      key_pair_ctx, private_key, private_key_length, true);
  themis_status_t res1 = soter_rsa_key_pair_gen_export_key(
      key_pair_ctx, public_key, public_key_length, false);
...
themis_status_t themis_gen_ec_key_pair(uint8_t *private_key,
                                       ...
                                       uint8_t *public_key,
                                       size_t *public_key_length) {
  return themis_gen_key_pair(SOTER_SIGN_ecdsa_none_pkcs8, private_key,
                             private_key_length, public_key, public_key_length);
}
...
themis_status_t themis_secure_message_wrap(
  ...
  if (public_key == NULL && public_key_length == 0) {
    ...
    ctx = themis_secure_message_encrypter_init(private_key, private_key_length,
...
themis_status_t themis_secure_message_unwrap(
  ...
  themis_secure_message_hdr_t *message_hdr =
      (themis_secure_message_hdr_t *)wrapped_message;
  THEMIS_CHECK_PARAM(
      IS_THEMIS_SECURE_MESSAGE_SIGNED(message_hdr->message_type) ||
      IS_THEMIS_SECURE_MESSAGE_ENCRYPTED(message_hdr->message_type));
  THEMIS_CHECK_PARAM(wrapped_message_length >=
                     THEMIS_SECURE_MESSAGE_LENGTH(message_hdr));
  if (IS_THEMIS_SECURE_MESSAGE_SIGNED(message_hdr->message_type)) {
    ...
    ctx = themis_secure_message_decrypter_init(private_key, private_key_length,
                                               public_key, public_key_length);
