 ...
 * @brief Secure message is a lightweight service which can help to deliver some
 * message or data to your peer in a secure manner.
 * @{
 */

/**
 * @brief generate RSA key pair
 * @param [out] private_key buffer for private key to store. May be set to NULL
 * for private key length determination
 * @param [in, out] private_key_length length of private_key
 * @param [out] public_key buffer for public key to store. May be set to NULL
 * for public key length determination
 * @param [in, out] public_key_length length of public key
 * @return THEMIS_SUCCESS on success or THEMIS_FAIL on failure
 * @note If private_key==NULL or public_key==NULL or private_key_length is not
 * enought for private key storage or public_key_length is not enought for
 * public key storage then THEMIS_BUFFER_TOO_SMALL will return and
 * private_key_length and public_key_length will store lengths of buffers needed
 * for private key and public key store respectively
 */
 ...
 * @param [out] wrapped_message buffer for wrapped message store. May be set to
 * NULL for wrapped message length determination
 * @param [in, out] wrapped_message_length length of wrapped_message
 * @return THEMIS_SUCCESS on success or THEMIS_FAIL on failure
 * @note If wrapped_message==NULL or wrapped_message_length is not enought for
 * wrapped message storage then THEMIS_BUFFER_TOO_SMALL will return and
 * wrapped_message_length will store length of buffer needed for wrapped message
 * store
 */
 ...
 * @param [out] message buffer for plain message store. May be set to NULL for
 * plain message length determination
 * @param [in, out] message_length length of message
 * @return THEMIS_SUCCESS on success or THEMIS_FAIL on failure
 * @note If message==NULL or message_length is not enought for plain message
 * storage then THEMIS_BUFFER_TOO_SMALL will return and message_length will
 * store length of buffer needed for plain message store
 */
