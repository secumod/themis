...
secure_session_proceed_client(secure_session_t *session_ctx, const void *data,
                              size_t data_length, void *output,
                              size_t *output_length);
static themis_status_t
secure_session_finish_client(secure_session_t *session_ctx, const void *data,
                             size_t data_length, void *output,
                             size_t *output_length);
static themis_status_t
secure_session_finish_server(secure_session_t *session_ctx, const void *data,
                             size_t data_length, void *output,
                             size_t *output_length);
...
themis_status_t
secure_session_init(secure_session_t *session_ctx, const void *id,
                    size_t id_length, const void *sign_key,
  ...
  res = secure_session_peer_init(&(session_ctx->we), id, id_length, NULL, 0,
  ...
  soter_status =
      soter_asym_ka_init(&(session_ctx->ecdh_ctx), SOTER_ASYM_KA_EC_P256);
...
secure_session_t *
secure_session_create(const void *id, size_t id_length, const void *sign_key,
                      size_t sign_key_length,
                      const secure_session_user_callbacks_t *user_callbacks) {
  ...
  if (THEMIS_SUCCESS == secure_session_init(ctx, id, id_length, sign_key,
                                            sign_key_length, user_callbacks)) {
    return ctx;
  } else {
    free(ctx);
    return NULL;
...
themis_status_t
secure_session_generate_connect_request(secure_session_t *session_ctx,
  ...
  soter_status = soter_asym_ka_export_key(&(session_ctx->ecdh_ctx), NULL,
                                          &ecdh_key_length, false);
  if (THEMIS_BUFFER_TOO_SMALL != soter_status) {
    return soter_status;
  }
  res = compute_signature(session_ctx->we.sign_key,
                          session_ctx->we.sign_key_length, NULL, 0, NULL,
                          &signature_length);
  if (THEMIS_BUFFER_TOO_SMALL != res) {
    return res;
  }
  length_to_send = 2 * sizeof(soter_container_hdr_t) +
  ...
  memcpy(soter_container_data(container), session_ctx->we.id,
  ...
  soter_status = soter_asym_ka_export_key(
      &(session_ctx->ecdh_ctx),
  ...
  sign_data.data = data_to_send + (2 * sizeof(soter_container_hdr_t)) +
                   session_ctx->we.id_length;
  sign_data.length = ecdh_key_length;

  /* Actual signature may be 1 or 2 bytes less than reported above because
   * leading zeroes are stripped */
  length_to_send -= signature_length;

  res = compute_signature(session_ctx->we.sign_key,
                          session_ctx->we.sign_key_length, &sign_data, 1,
                          data_to_send + (2 * sizeof(soter_container_hdr_t)) +
                              session_ctx->we.id_length + ecdh_key_length,
                          &signature_length);
  if (THEMIS_SUCCESS != res) {
  ...
  *output_length = length_to_send;
  ...
  soter_container_set_data_size(container,
                                length_to_send - sizeof(soter_container_hdr_t));
  soter_update_container_checksum(container);
  ...
  if (session_ctx->user_callbacks->state_changed) {
    session_ctx->user_callbacks->state_changed(
        STATE_NEGOTIATING, session_ctx->user_callbacks->user_data);
  }
...
themis_status_t secure_session_connect(secure_session_t *session_ctx) {
  ...
  themis_status_t res = secure_session_generate_connect_request(
      session_ctx, request, &request_length);
  ...
  if (THEMIS_BUFFER_TOO_SMALL == res) {
    ...
    res = secure_session_generate_connect_request(session_ctx, request,
                                                  &request_length);
  }

  if (THEMIS_SUCCESS == res) {
    ssize_t bytes_sent = session_ctx->user_callbacks->send_data(
...
static themis_status_t secure_session_accept(secure_session_t *session_ctx,
  ...
  if (memcmp(proto_message->tag, THEMIS_SESSION_PROTO_TAG,
             SOTER_CONTAINER_TAG_LENGTH)) {
    return THEMIS_INVALID_PARAMETER;
  }

  if (data_length < (soter_container_data_size(proto_message) +
                     sizeof(soter_container_hdr_t))) {
    return THEMIS_INVALID_PARAMETER;
  }
  ...
  peer_id =
      (const soter_container_hdr_t *)soter_container_const_data(proto_message);
  ...
  peer_ecdh_key =
      (const soter_container_hdr_t *)(soter_container_const_data(peer_id) +
  ...
  peer_ecdh_key_length =
      soter_container_data_size(peer_ecdh_key) + sizeof(soter_container_hdr_t);

  signature = (const uint8_t *)peer_ecdh_key + peer_ecdh_key_length;
  signature_length = (const uint8_t *)data +
  ...
  if (session_ctx->user_callbacks->get_public_key_for_id(
          ...
          soter_container_data_size(peer_id), sign_key, sizeof(sign_key),
  ...
  res = verify_signature(peer_sign_key, sign_key_length, sign_data, 1,
  ...
  res = secure_session_peer_init(
      &(session_ctx->peer), soter_container_const_data(peer_id),
      soter_container_data_size(peer_id), peer_ecdh_key, peer_ecdh_key_length,
      peer_sign_key, sign_key_length);
  if (THEMIS_SUCCESS != res) {
    goto err;
  }
  ...
  sign_data[0].data = data_to_send + (2 * sizeof(soter_container_hdr_t)) +
                      session_ctx->we.id_length;
  ...
  res = compute_signature(session_ctx->we.sign_key,
                          session_ctx->we.sign_key_length, sign_data, 4,
                          data_to_send + (2 * sizeof(soter_container_hdr_t)) +
                              session_ctx->we.id_length + ecdh_key_length,
                          &signature_length);
  if (THEMIS_SUCCESS != res) {
...
secure_session_proceed_client(secure_session_t *session_ctx, const void *data,
  ...
  soter_status = soter_asym_ka_export_key(&(session_ctx->ecdh_ctx), ecdh_key,
                                          &ecdh_key_length, false);
  if (THEMIS_SUCCESS != soter_status) {
    return soter_status;
  }
  ...
  res = verify_signature(peer_sign_key, sign_key_length, sign_data, 4,
                         signature, signature_length);
  if (THEMIS_SUCCESS != res) {
    return res;
  }
  ...
  res = soter_kdf(NULL, 0, SESSION_ID_GENERATION_LABEL, sign_data, 4,
  ...
  res = soter_asym_ka_derive(&(session_ctx->ecdh_ctx), peer_ecdh_key,
                             peer_ecdh_key_length, sign_key, &sign_key_length);
  if (THEMIS_SUCCESS != res) {
    goto err;
  }
  ...
  res =
      soter_kdf(sign_key, sign_key_length, SESSION_MASTER_KEY_GENERATION_LABEL,
  ...
  res = compute_mac(session_ctx->session_master_key,
                    sizeof(session_ctx->session_master_key), NULL, 0, NULL,
                    &sign_key_length);
  if (THEMIS_BUFFER_TOO_SMALL != res) {
    goto err;
  }
  ...
  length_to_send =
      sizeof(soter_container_hdr_t) + signature_length + sign_key_length;
  if ((NULL == output) || (*output_length < length_to_send)) {
    *output_length = length_to_send;
  ...
  res = compute_mac(session_ctx->session_master_key,
                    sizeof(session_ctx->session_master_key), sign_data, 2, mac,
                    &sign_key_length);
  if (THEMIS_SUCCESS != res) {
    goto err;
  }
...
secure_session_finish_server(secure_session_t *session_ctx, const void *data,
  ...
  res =
      compute_mac(session_ctx->peer.ecdh_key, session_ctx->peer.ecdh_key_length,
                  NULL, 0, NULL, &mac_length);
  if (THEMIS_BUFFER_TOO_SMALL != res) {
    return res;
  }
  ...
  res = soter_asym_ka_export_key(&(session_ctx->ecdh_ctx), ecdh_key,
                                 &ecdh_key_length, false);
  if (THEMIS_SUCCESS != res) {
    return res;
  }
  ...
  res = verify_signature(session_ctx->peer.sign_key,
                         session_ctx->peer.sign_key_length, sign_data, 4,
                         signature, signature_length);
  if (THEMIS_SUCCESS != res) {
    return res;
  }

  res = soter_asym_ka_derive(
      &(session_ctx->ecdh_ctx), session_ctx->peer.ecdh_key,
      session_ctx->peer.ecdh_key_length, shared_secret, &shared_secret_length);
  if (THEMIS_SUCCESS != res) {
    return res;
  }
  ...
  res = soter_kdf(shared_secret, shared_secret_length,
                  SESSION_MASTER_KEY_GENERATION_LABEL, sign_data, 1,
  ...
  res = compute_mac(session_ctx->session_master_key,
                    sizeof(session_ctx->session_master_key), sign_data, 2,
                    soter_container_data(response_message), &ecdh_key_length);
  if (THEMIS_SUCCESS != res) {
    return res;
  }
  ...
  *output_length = ecdh_key_length + sizeof(soter_container_hdr_t);
  ...
  memcpy(response_message->tag, THEMIS_SESSION_PROTO_TAG,
         SOTER_CONTAINER_TAG_LENGTH);
  soter_container_set_data_size(response_message, ecdh_key_length);
  soter_update_container_checksum(response_message);

  memcpy(output, ecdh_key, soter_container_data_size(response_message) +
                               sizeof(soter_container_hdr_t));
  ...
  if (session_ctx->user_callbacks->state_changed) {
    session_ctx->user_callbacks->state_changed(
        STATE_ESTABLISHED, session_ctx->user_callbacks->user_data);
  }
...
ssize_t secure_session_send(secure_session_t *session_ctx, const void *message,
  ...
  res =
      secure_session_wrap(session_ctx, message, message_length, out, &out_size);
  if (THEMIS_SUCCESS != res) {
  ...
  bytes_sent = session_ctx->user_callbacks->send_data(
      out, out_size, session_ctx->user_callbacks->user_data);
...
ssize_t secure_session_receive(secure_session_t *session_ctx, void *message,
                               size_t message_length) {
  uint8_t stack_buf[2048];
  uint8_t *in;
  ...
  if (!secure_session_is_established(session_ctx)) {
    ...
    /* If user is expecting to receive message_length, then we need a buffer to
     * receive at least WRAPPED_SIZE(message_length) */
  ...
  res = session_ctx->user_callbacks->receive_data(
      in, in_size, session_ctx->user_callbacks->user_data);
  ...
  if (!secure_session_is_established(session_ctx)) {
    ...
    res = secure_session_unwrap(session_ctx, in, bytes_received, message,
                                &message_length);
    if (THEMIS_SUCCESS == res) {
...
themis_status_t
secure_session_get_remote_id(const secure_session_t *session_ctx, uint8_t *id,
                             size_t *id_length) {
