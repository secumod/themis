...
/* How much sequence number may differ from expected for message to be
 * considered valid */
#define SEQ_MAX_DIFF 8

/* How much timestamp may differ from expected for message to be considered
 * valid */
...
themis_status_t secure_session_wrap(secure_session_t *session_ctx,
  ...
  if ((NULL == wrapped_message) ||
      (WRAPPED_SIZE(message_length) > *wrapped_message_length)) {
  ...
  *length = htonl(message_length + sizeof(uint32_t) + sizeof(uint64_t));
  ...
  res = encrypt_gcm(
      session_ctx->out_cipher_key, sizeof(session_ctx->out_cipher_key), iv,
      CIPHER_MAX_BLOCK_SIZE, length,
      message_length + sizeof(uint32_t) + sizeof(uint64_t) + sizeof(uint32_t),
      length, message_length + sizeof(uint32_t) + sizeof(uint64_t) +
                  sizeof(uint32_t) + CIPHER_AUTH_TAG_SIZE);
  if (THEMIS_SUCCESS != res) {
    return res;
  }
...
themis_status_t secure_session_unwrap(secure_session_t *session_ctx,
  ...
  uint8_t
      message_header[sizeof(uint32_t) + sizeof(uint32_t) + sizeof(uint64_t)];
  ...
  if ((NULL == session_ctx) || (NULL == wrapped_message) ||
      (NULL == message_length)) {
    return THEMIS_INVALID_PARAMETER;
  }

  if (!secure_session_is_established(session_ctx)) {
    res = session_ctx->state_handler(session_ctx, wrapped_message,
  ...
  if ((NULL == message) ||
      (UNWRAPPED_SIZE(wrapped_message_length) > *message_length)) {
  ...
  /*{
          ...
          ts = be64toh(*((time_t *)(message_header + sizeof(uint32_t) +
  sizeof(uint32_t))));
  }*/

  res = soter_sym_aead_decrypt_update(sym_ctx, iv + CIPHER_MAX_BLOCK_SIZE,
                                      sizeof(message_header), message_header,
                                      &message_header_size);
  if (THEMIS_SUCCESS != res) {
    goto err;
  }
  ...
  seq = ntohl(*((uint32_t *)(message_header + sizeof(uint32_t))));
  ts = be64toh(
      *((uint64_t *)(message_header + sizeof(uint32_t) + sizeof(uint32_t))));

  if (length >
      (UNWRAPPED_SIZE(wrapped_message_length) + sizeof(uint32_t) + 8)) {
  ...
  if ((ts < (uint64_t)(curr_time - TS_MAX_DIFF)) ||
      (ts > (uint64_t)(curr_time + TS_MAX_DIFF))) {
    res = THEMIS_INVALID_PARAMETER;
    goto err;
  }
  ...
  *message_length = length - (sizeof(uint32_t) + sizeof(uint64_t));
  ...
  /*{
          ...
          {
                  ((uint8_t *)message)[i] = iv[CIPHER_MAX_BLOCK_SIZE +
  sizeof(message_header) + i] ^ 0xff;
          }
  }*/
