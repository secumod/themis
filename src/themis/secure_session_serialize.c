...
#define SESSION_CTX_SERIZALIZED_SIZE(_CTX_)                                    \
  (sizeof(_CTX_->session_id) + sizeof(_CTX_->is_client) +                      \
   sizeof(_CTX_->session_master_key) + sizeof(_CTX_->out_seq) +                \
   sizeof(_CTX_->in_seq))
...
themis_status_t secure_session_save(const secure_session_t *session_ctx,
  ...
  if (session_ctx->state_handler) {
    /* Key agreement is not complete. We cannot save session state at this
     * stage. */
    return THEMIS_INVALID_PARAMETER;
  }
  ...
  if ((!out) || (*out_length < (sizeof(soter_container_hdr_t) +
                                SESSION_CTX_SERIZALIZED_SIZE(session_ctx)))) {
    *out_length = (sizeof(soter_container_hdr_t) +
                   SESSION_CTX_SERIZALIZED_SIZE(session_ctx));
  ...
  *curr = htonl(session_ctx->is_client);
  ...
  /* master_key */
  memcpy(curr, session_ctx->session_master_key,
...
secure_session_load(secure_session_t *session_ctx, const void *in,
  ...
  if (in_length <
      (sizeof(soter_container_hdr_t) + soter_container_data_size(hdr))) {
    return THEMIS_INVALID_PARAMETER;
  }
  ...
  memcpy(session_ctx->session_master_key, curr,
         sizeof(session_ctx->session_master_key));
  curr += sizeof(session_ctx->session_master_key) / sizeof(uint32_t);

  /* We have to derive session keys before extracting sequence numbers, because
   * this function overwrites them */
