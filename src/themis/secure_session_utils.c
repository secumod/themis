...
soter_sign_alg_t get_key_sign_type(const void *sign_key,
...
soter_sign_alg_t get_peer_key_sign_type(const void *sign_key,
                                        size_t sign_key_length) {
  const soter_container_hdr_t *key = sign_key;
...
themis_status_t compute_signature(const void *sign_key, size_t sign_key_length,
                                  ...
                                  size_t sign_data_count, void *signature,
  ...
  sign_ctx = soter_sign_create(get_key_sign_type(sign_key, sign_key_length),
                               sign_key, sign_key_length, NULL, 0);
  if (!sign_ctx) {
    return THEMIS_FAIL;
  }
  ...
  if (sign_data && signature) {
    for (i = 0; i < sign_data_count; i++) {
      soter_status =
          soter_sign_update(sign_ctx, sign_data[i].data, sign_data[i].length);
...
themis_status_t verify_signature(const void *verify_key,
                                 ...
                                 const soter_kdf_context_buf_t *sign_data,
                                 size_t sign_data_count, const void *signature,
                                 size_t signature_length) {
  soter_sign_ctx_t *sign_ctx = NULL;
  soter_status_t soter_status;
  size_t i;
  ...
  sign_ctx =
      soter_verify_create(get_peer_key_sign_type(verify_key, verify_key_length),
                          NULL, 0, verify_key, verify_key_length);
  if (!sign_ctx) {
    return SOTER_FAIL;
  }
  ...
  for (i = 0; i < sign_data_count; i++) {
    soter_status =
        soter_verify_update(sign_ctx, sign_data[i].data, sign_data[i].length);
    if (THEMIS_SUCCESS != soter_status) {
      goto err;
    }
  }
  ...
  soter_status =
      soter_verify_final(sign_ctx, (void *)signature, signature_length);
...
themis_status_t compute_mac(const void *key, size_t key_length,
                            const soter_kdf_context_buf_t *data,
                            size_t data_count, void *mac, size_t *mac_length) {
  soter_hmac_ctx_t *mac_ctx =
      soter_hmac_create(SOTER_HASH_SHA256, key, key_length);
  if (!mac_ctx) {
    return THEMIS_FAIL;
  }
  soter_status_t soter_status;
...
themis_status_t verify_mac(const void *key, size_t key_length,
                           const soter_kdf_context_buf_t *data,
                           size_t data_count, const void *mac,
                           size_t mac_length) {
  uint8_t computed_mac[MAX_HMAC_SIZE];
  size_t computed_mac_length = sizeof(computed_mac);

  themis_status_t res = compute_mac(key, key_length, data, data_count,
                                    computed_mac, &computed_mac_length);
...
themis_status_t encrypt_gcm(const void *key, size_t key_length, const void *iv,
                            size_t iv_length, const void *in, size_t in_length,
  ...
  ctx = soter_sym_aead_encrypt_create(SOTER_SYM_AES_GCM |
                                          SOTER_SYM_256_KEY_LENGTH,
                                      key, key_length, NULL, 0, iv, iv_length);
  if (NULL == ctx) {
    return THEMIS_FAIL;
  }

  res =
      soter_sym_aead_encrypt_update(ctx, in, in_length, out, &bytes_encrypted);
  if (THEMIS_SUCCESS != res) {
    goto err;
  }
  ...
  res = soter_sym_aead_encrypt_final(ctx, ((uint8_t *)out) + in_length,
                                     &out_length);
  if (THEMIS_SUCCESS != res) {
    goto err;
  }
...
/*themis_status_t decrypt_gcm(const void *key, size_t key_length, const void
*iv, size_t iv_length, const void *in, size_t in_length, void *out, size_t
out_length)
{
        const uint8_t *input = in;
        uint8_t *output = out;
...
themis_status_t
secure_session_derive_message_keys(secure_session_t *session_ctx) {
  ...
  soter_kdf_context_buf_t context = {
      (const uint8_t *)&(session_ctx->session_id),
  ...
  res = soter_kdf(session_ctx->session_master_key, SESSION_MASTER_KEY_LENGTH,
                  out_key_label, &context, 1, session_ctx->out_cipher_key,
  ...
  res = soter_kdf(session_ctx->session_master_key, SESSION_MASTER_KEY_LENGTH,
                  in_key_label, &context, 1, session_ctx->in_cipher_key,
                  SESSION_MESSAGE_KEY_LENGTH);
  if (THEMIS_SUCCESS != res) {
    return res;
  }
  ...
  res = soter_kdf(session_ctx->session_master_key, SESSION_MASTER_KEY_LENGTH,
                  out_seq_label, &context, 1, &(session_ctx->out_seq),
  ...
  res = soter_kdf(session_ctx->session_master_key, SESSION_MASTER_KEY_LENGTH,
                  in_seq_label, &context, 1, &(session_ctx->in_seq),
